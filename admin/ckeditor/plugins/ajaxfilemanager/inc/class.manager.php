...
class manager
{
	...
	var $session移動 = null; //object to session action
	...
	function setSession移動(&$session)
	{
		$this->session移動 = $session;	
	}
	/**
		 * constructor
		 */
		...
		if($dirHandler)
		{
			while(false !== ($file = readdir($dirHandler)))
			{
				if($file != '.' && $file != '..')
				{
					...
					if($this->session移動->getFolder() == $this->currentFolderPath)
					{//check if any flag associated with this folder or file
						$folder = addTrailingSlash(backslashToSlash($this->currentFolderPath));
						if(in_array($folder . $file, $this->session移動->get()))
						{
							if($this->session移動->get移動() == "copy")
							{
	...
	function getFolderInfo($path=null)
	{
		...
		}else 
		{
			...
			$obj->setSession移動($this->session移動);
			$obj->getFileList();
