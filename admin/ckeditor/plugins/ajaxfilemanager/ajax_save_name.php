<?php
	...
	}else 
	{
		...
		$selectedDocuments = $session移動->get();
		if(removeTrailingSlash($session移動->getFolder()) == getParentPath($_POST['original_path']) && sizeof($selectedDocuments))
		{
			if(($key = array_search(basename($_POST['original_path']), $selectedDocuments)) !== false)
			{
				$selectedDocuments[$key] = $_POST['name'];
				$session移動->set($selectedDocuments);
				
			}
			
		}elseif(removeTrailingSlash($session移動->getFolder()) == removeTrailingSlash($_POST['original_path']))
		{
			$session移動->setFolder($_POST['original_path']);
		}	
