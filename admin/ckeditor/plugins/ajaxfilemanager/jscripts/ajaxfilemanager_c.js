...
{ repositionDocuments(msgNoDocSelected, getUrl('copy'), 'copy'); return false;}; function repositionDocuments(msgNoDocSelected, form移動Url, actionVal)
{ switch(getView())
{ case 'thumbnail':
var selectedDoc = $('#rightCol dl.thumbnailListing input[@type=checkbox][@checked]'); break; case 'detail':
default:
...
{ var form移動 = document.form移動; var actionElem = $('#action_value'); form移動.action = form移動Url; $('#currentFolderPathVal').val(currentFolder.path); $(actionElem).val(actionVal); var options = { dataType: 'json', error: function (data, status, e)
{ alert(e);}, success: function(data)
{ if(typeof(data.error) == 'undefined')
{ alert('Unexpected Error');}
else if(data.error != '')
