...
*/
	...
	*/
	function getModeValue()
	{
		...
		for(var i = 0; i < document.form移動.mode.length; i++)
		{
			if(document.form移動.mode[i].checked || i == 0)
			{
				CheckedElem = document.form移動.mode[i];
			}
		}
				...
				*/
				...
				switch(mode)
				{
					case "resize":
						...
						enableResize(document.form移動.constraint.checked);
						break;
					case "crop":						
						switch($('#image_mode').val())
						{
	...
	*/
	...
	{
		hideHandlers();	
		if(document.form移動.constraint.checked)
		{
	...
	*/
	...
	{
		$('#form移動 input[@value=' + $('#image_mode').val() + ']').attr('checked', 'checked');	
	};
	/**
