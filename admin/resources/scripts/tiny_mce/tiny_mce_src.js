...
(function(tinymce) {
	...
	tinymce.create('tinymce.ui.SplitButton:tinymce.ui.MenuButton', {
		...
		renderHTML : function() {
			...
			if (s.image)
				h1 = DOM.createHTML('img ', {src : s.image, 'class' : 'mce移動 ' + s['class']});
			else
				h1 = DOM.createHTML('span', {'class' : 'mce移動 ' + s['class']}, '');

			h += '<td>' + DOM.createHTML('a', {id : t.id + '_action', href : 'javascript:;', 'class' : 'mce移動 ' + s['class'], onclick : "return false;", onmousedown : 'return false;', title : s.title}, h1) + '</td>';
	...
	tinymce.Formatter = function(ed) {
			...
			if (format) {
				if (node) {
					...
					if (!selection.isCollapsed() || !format.inline) {
						...
						performCaret移動('apply', name, vars);
				}
			}
		};
		...
		function remove(name, vars, node) {
			...
			if (!selection.isCollapsed() || !format.inline) {
				...
				performCaret移動('remove', name, vars);
		};
		...
		function performCaret移動(type, name, vars) {
